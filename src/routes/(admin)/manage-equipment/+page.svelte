	...
	function filteredEquipments() {
		return equipments.filter((equipment) => {
			...
			const matchesDepartment =
				selectedDepartment === 'All' ? true : equipment.department === selectedDepartment;
...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<h1 class="text-3xl font-bold text-blue-900">Manage Equipment</h1>

			<p class="mt-2 text-slate-600">Search, add, edit and delete laboratory equipment.</p>
		</div>

		<!-- Search + Add Button -->

		<div class="mb-6">
			<EquipmentToolbar {search} onSearch={handleSearch} showButton={false} />
		</div>

		<!-- Department Filter -->

		<div class="mb-6">
			<DepartmentFilter {selectedDepartment} onSelect={handleDepartment} />
		</div>

		<!-- Equipment Table -->
		{#if loading}
			<div class="rounded-3xl bg-white p-10 text-center shadow-lg">
				<p class="text-lg font-semibold text-slate-600">Loading equipments...</p>
			</div>
		{:else}
			<EquipmentTable equipments={filteredEquipments()} />
		{/if}
		<!-- Quick Actions -->

		<div class="mt-8 rounded-3xl bg-white p-6 shadow-lg">
			<h2 class="mb-6 text-2xl font-bold text-blue-900">Quick Actions</h2>
			...
			<div class="grid gap-6 md:grid-cols-3">
				...
				>
					...
					<h3 class="mt-4 text-xl font-bold">Add Equipment</h3>

					<p class="mt-2 text-sm">Add a new equipment.</p>
				</button>
				...
				>
					...
					<h3 class="mt-4 text-xl font-bold">Edit Equipment</h3>

					<p class="mt-2 text-sm">Open edit equipment page.</p>
				</button>
				...
				>
					...
					<h3 class="mt-4 text-xl font-bold">Delete Equipment</h3>

					<p class="mt-2 text-sm">Delete equipment permanently.</p>
				</button>
			</div>
		</div>
	</main>
