<script>
	...
	async function handleAccept(request) {
		const equipmentRef = doc(db, 'equipments', request.equipmentId);
	...
	async function loadRequests() {
		...
		try {
			...
			requests = snapshot.docs.map((document) => {
				...
				const bookedDate = data.createdAt?.toDate();
				...
				return {
					...
					requestDate: bookedDate?.toLocaleDateString(),

					requestTime: bookedDate?.toLocaleTimeString()
				};
			});
		} catch (error) {
			console.error(error);
...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<h1 class="text-3xl font-bold text-blue-900">Booking Requests</h1>
		...
		{#if loading}
			<div class="rounded-3xl bg-white p-10 text-center shadow-lg">
				<p class="text-lg font-semibold text-slate-600">Loading booking requests...</p>
			</div>
		{:else}
			<BookingRequestTable {requests} onAccept={handleAccept} onReject={handleReject} />
		{/if}
