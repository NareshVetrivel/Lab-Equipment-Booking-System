<script>
	...
	async function deleteEquipment(equipment) {
		try {
			await deleteDoc(doc(db, 'equipments', equipment.id));

			equipments = equipments.filter((item) => item.id !== equipment.id);
...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<h1 class="text-3xl font-bold text-blue-900">Delete Equipment</h1>

			<p class="mt-2 text-slate-600">Permanently remove laboratory equipment from the inventory.</p>
		</div>
		...
		<div class="mb-6">
			<EquipmentToolbar {search} onSearch={handleSearch} buttonText="" onAdd={() => {}} />
		</div>
			...
			{#if filteredEquipments().length > 0}
				<div class="grid gap-8 sm:grid-cols-2 xl:grid-cols-3">
					{#each filteredEquipments() as equipment (equipment.id)}
						<DeleteEquipmentCard {equipment} onDelete={openDeleteModal} />
					{/each}
				</div>
			{:else}
				<div class="rounded-3xl bg-white p-16 text-center shadow-lg">
					...
					<h2 class="mt-6 text-3xl font-bold text-slate-700">No Equipment Found</h2>

					<p class="mt-3 text-slate-500">No equipment matches the selected search or department.</p>
				</div>
			{/if}
		{/if}
	</main>
