...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			...
			<p class="mt-2 text-slate-600">Select an equipment to edit its information.</p>
		</div>
			...
			{#if filteredEquipments().length > 0}
				<div class="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
					{#each filteredEquipments() as equipment (equipment.id)}
						<EditEquipmentCard {equipment} onEdit={openEdit} />
					{/each}
				</div>
			{:else}
				<div class="rounded-3xl bg-white px-8 py-20 text-center shadow-lg">
