<script>
	...
	const returnDaysRegex = /^[1-9][0-9]?$/;

	const departments = ['Computer Science', 'Physics', 'Chemistry', 'Botany', 'Zoology'];
	...
	async function loadEquipment() {
		...
		const document = await getDoc(doc(db, 'equipments', id));
		...
		equipment = {
			...
			returnWithinDays: data.returnWithinDays ?? 0,
	...
	async function updateEquipment() {
		try {
			...
			if (Number(equipment.available) > Number(equipment.total)) {
				loading = false;

				errorMessage = 'Available quantity cannot be greater than total quantity.';

				return;
			}

			if (!returnDaysRegex.test(String(equipment.returnWithinDays))) {
				loading = false;

				errorMessage = 'Return days must be between 1 and 99.';
			...
			await updateDoc(doc(db, 'equipments', id), {
				...
				image: imagePreview,
				status: Number(equipment.available) > 0 ? 'Available' : 'Out of Stock',
...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<h1 class="text-3xl font-bold text-blue-900">Edit Equipment</h1>

			<p class="mt-2 text-slate-600">Update laboratory equipment details.</p>
		</div>
		...
		<div class="rounded-3xl bg-white p-8 shadow-lg">
			<div class="grid gap-6 md:grid-cols-2">
				...
				<div class="md:col-span-2">
					<label class="mb-2 block font-semibold text-slate-700"> Equipment Image </label>
				...
				<div>
					<label class="mb-2 block font-semibold text-slate-700"> Equipment Name </label>
				...
				<div>
					<label class="mb-2 block font-semibold text-slate-700"> Department </label>
				...
				<div>
					<label class="mb-2 block font-semibold text-slate-700"> Total Quantity </label>
				...
				<div>
					<label class="mb-2 block font-semibold text-slate-700"> Available Quantity </label>
				...
				<div>
					<label class="mb-2 block font-semibold text-slate-700"> Return Within (Days) </label>
					...
					<input
						...
						oninput={(event) => {
							...
							equipment.returnWithinDays = Number(target.value);
						}}
						class="w-full rounded-xl border border-slate-300 p-3 outline-none focus:border-blue-600"
					/>
				</div>
