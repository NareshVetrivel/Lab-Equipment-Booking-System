<script>
	...
	const equipmentNameRegex = /^[A-Za-z0-9\s()-]+$/;
	...
	async function handleImage(event) {
		...
		if (file.size > 2 * 1024 * 1024) {
			errorMessage = 'Image size should not exceed 2 MB.';

			return;
		}

		imagePreview = await compressImage(file);
	}
...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<h1 class="text-3xl font-bold text-blue-900">Add Equipment</h1>

			<p class="mt-2 text-slate-600">Add a new laboratory equipment to the inventory.</p>
		</div>
		{#if errorMessage}
