...
<div class="flex min-h-screen flex-col bg-gradient-to-br from-blue-50 via-white to-sky-100">
	...
	<main class="relative flex flex-1 items-center justify-center px-4 py-10">
		...
		<div class="absolute inset-0 overflow-hidden pointer-events-none">
			...
			<div class="absolute right-0 bottom-10 h-96 w-96 rounded-full bg-sky-400/20 blur-3xl"></div>
		</div>
		...
		>
			<div class="mb-8 text-center">
				<h1 class="text-4xl font-bold text-blue-950">Student Login</h1>
			</div>
			{#if errorMessage}
			...
			{#if successMessage}
				<div
					class="mb-4 rounded-xl bg-green-100 p-3 text-center text-sm font-medium text-green-700"
				>
					{successMessage}
				</div>
			{/if}
			...
			>
				...
				<div class="flex flex-col gap-2">
					<label for="email" class="text-sm font-semibold text-slate-700"> College Email </label>
				...
				<p class="text-center text-sm text-slate-600">
					...
					<a href={resolve('/student-signup')} class="font-semibold text-blue-700 hover:underline">
						Sign Up
					</a>
				</p>
			</form>
