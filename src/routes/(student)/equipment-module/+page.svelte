<script>
	...
	const categories = ['All', 'Computer Science', 'Physics', 'Chemistry', 'Botany', 'Zoology'];
	...
	const filteredEquipments = $derived.by(() => {
		return equipments.filter((equipment) => {
			...
			const categoryMatch = category === 'All' || equipment.department === category;
...
<div class="min-h-screen bg-slate-100">
	...
	<main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
		...
		<div class="mb-8 text-center">
			<h1 class="text-2xl font-bold text-blue-900 sm:text-3xl lg:text-4xl">Equipment Module</h1>

			<p class="mt-2 text-sm text-slate-600 sm:text-base">Browse and Book Laboratory Equipment</p>
		</div>
		...
		{:else if filteredEquipments.length === 0}
			<div class="rounded-3xl bg-white px-6 py-16 text-center shadow-lg">
				...
				<h2 class="mt-4 text-xl font-bold text-slate-700">No Equipment Found</h2>
