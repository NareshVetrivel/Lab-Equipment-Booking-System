<script>
	...
	async function saveProfile() {
		...
		if (!studentNameRegex.test(studentName)) {
			errorMessage = 'Student name can contain only letters and spaces.';
			return;
		}
		...
		if (!academicYearRegex.test(academicYear)) {
			errorMessage = 'Academic Year must be in YYYY-YYYY format.';
			return;
		}
		...
		if (endYear !== startYear + 2) {
			errorMessage = 'Academic Year must be like 2025-2027.';
			return;
		}
		...
		if (result.success) {
			...
			switch (result.message) {
				...
				case 'Admission number already exists.':
					errorMessage = 'This admission number is already registered.';
					break;
	...
	<main class="flex flex-1 items-center justify-center px-4 py-10">
		...
		>
			<div class="mb-8 text-center">
				<h1 class="text-4xl font-bold text-blue-950">Student Registration</h1>

				<p class="mt-2 text-slate-600">Complete your profile to continue</p>
				{#if errorMessage}
				...
				{#if successMessage}
					<div
						class="mt-5 rounded-xl bg-green-100 p-3 text-center text-sm font-medium text-green-700"
					>
						{successMessage}
					</div>
				{/if}
			...
			>
				...
				<div>
					<label class="mb-2 block font-semibold"> Student Name </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Admission Number </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Department </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Year of Studying </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Academic Year </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Date of Birth </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Phone Number </label>
				...
				<div>
					<label class="mb-2 block font-semibold"> Gender </label>
