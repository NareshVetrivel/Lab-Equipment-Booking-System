...
export async function createBooking(booking) {
	try {
		...
		const snapshot = await getDocs(duplicateBookingQuery);

		const studentDocument = await getDoc(doc(db, 'students', booking.studentId));
		...
		if (alreadyBooked) {
			return {
				...
				message: 'You have already requested this equipment.'
			};
		}
		await addDoc(collection(db, 'bookings'), {
			...
			returnWithinDays: Number(booking.returnWithinDays),
